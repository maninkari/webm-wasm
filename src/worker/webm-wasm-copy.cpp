...
WebmEncoder::WebmEncoder(int timebase_num, int timebase_den, unsigned int width, unsigned int height, unsigned int bitrate_kbps, bool realtime_, bool klive, val cb): realtime(realtime_) {
    std::cout << "WebmEncoder!!!!\n";
  if(!InitCodec(timebase_num, timebase_den, width, height, bitrate_kbps)) {
  std::cout << "WebmEncoder1!!!!\n";
    throw last_error;
  }
  if(!InitMkvWriter(klive, cb)) {
  std::cout << "WebmEncoder2!!!!\n";
    throw last_error;
  }
  if(!InitImageBuffer()) {
  std::cout << "WebmEncoder3!!!!\n";
    throw last_error;
  }
}

WebmEncoder::~WebmEncoder() {
    std::cout << "WebmEncoder dest!\n";
  vpx_img_free(img);
  delete main_segment;
  if(realtime) {
...
//  std::cout << strlen(rgba.c_str());
  std::cout << "addRGBAFrame\n";
  RGBAtoVPXImage((const uint8_t*) rgba.c_str());
  if(!EncodeFrame(img)) {
    std::cout << "addRGBAFrame failed\n";
    return false;
  }
  std::cout << "addRGBAFrame worked\n";
  if(realtime) {
  std::cout << "addRGBAFrame realtime start\n";
    ((MyMkvStreamWriter*)mkv_writer)->Notify();
    std::cout << "addRGBAFrame realtime after\n";
  }
  return true;
}
...
bool WebmEncoder::EncodeFrame(vpx_image_t *img) {
  ...
  std::cout << VPX_CODEC_OK;
  std::cout << "EncodeFrame4!\n";
...
bool WebmEncoder::InitMkvWriter(bool klive, val cb) {
    std::cout << "InitMkvWriter!\n";
  if(realtime) {
    mkv_writer = new MyMkvStreamWriter(cb);
    std::cout << "InitMkvWriter1!\n";
  } else {
    mkv_writer = new MyMkvWriter(cb);
    std::cout << "InitMkvWriter2!\n";
  }
  ...
  std::cout << "InitMkvWriter3!\n";
  return true;
}

bool WebmEncoder::InitImageBuffer() {
    std::cout << "InitImageBuffer!\n";
  img = vpx_img_alloc(
